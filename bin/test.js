...
async.parallel([
  function prodTest(cb) {
    shellExec('NODE_ENV="test-production" ./node_modules/mocha/bin/_mocha -R spec test/*/* -t 120000 -s 20000', {env: require("../test-production.json"), cwd: __dirname + '/..'}, function (err, stdout, stderr) {
      if(stderr){
        var json = {
  ...
  function prodTest(cb) {
    shellExec('NODE_ENV="test-staging" ./node_modules/mocha/bin/_mocha -R spec test/*/* -t 120000 -s 20000', {env: require("../test-staging.json"), cwd: __dirname + '/..'}, function (err, stdout, stderr) {
      if(stderr) {
        var json = {
